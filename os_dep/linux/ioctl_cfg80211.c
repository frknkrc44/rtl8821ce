...
u8 rtw_cfg80211_ch_switch_notify(_adapter *adapter, u8 ch, u8 bw, u8 offset, u8 ht, bool started)
{
	...
	if (started) {
		...
		cfg80211_ch_switch_started_notify(adapter->pnetdev, &chdef, 0, 0, false, 0xffff);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(6, 9, 0))
	cfg80211_ch_switch_notify(adapter->pnetdev, &chdef, 0, 0xffff);
...
int cfg80211_rtw_set_default_mgmt_key(struct wiphy *wiphy,
	struct net_device *ndev, int link_id, u8 key_index)
{
#define SET_DEF_KEY_PARAM_FMT " key_index=%d, link_id=%d"
#define SET_DEF_KEY_PARAM_ARG , key_index , link_id
#endif
